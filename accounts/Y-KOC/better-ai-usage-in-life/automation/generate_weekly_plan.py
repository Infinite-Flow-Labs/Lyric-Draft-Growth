...
ANGLE_OPTIONS = ["Contrarian take", "Pitfall review", "Before/after comparison", "Method breakdown", "Execution checklist"]
AUDIENCE_OPTIONS = ["Everyday users", "Students", "Creators", "Solo founders", "Non-technical operators"]
OUTCOME_OPTIONS = ["Save time", "Increase output", "Reduce rework", "Improve consistency", "Improve conversion efficiency"]
...
def source_directions(type_codes: list[str]) -> list[str]:
    mapping = {
        "A": "Work replacement benchmark posts (automation, task substitution, efficiency)",
        "B": "Result-driven benchmark posts (growth, conversion, delivery speed)",
        "C": "Capability amplification benchmark posts (leverage, parallel output, decision quality)",
        "D": "Experience benchmark posts (onboarding, friction, pitfalls, fixes)",
        "E": "Workflow benchmark posts (SOP, repeatable steps, system design)",
    }
    out = []
    seen = set()
    for t in type_codes:
    ...
    out.append("Topic reverse-search from same-day trends (timeliness)")
    out.append("Source mix: 60% benchmark creators + 40% topic reverse-search")
    return out
...
def main() -> int:
    ...
    for i in range(args.days):
        ...
        lines.append(f"## {d}")
        lines.append("- Source directions:")
        for s in source_directions(list(mix)):
            lines.append(f"  - {s}")
        lines.append("- 3 posts today:")
        for j, s in enumerate(slots, 1):
            lines.append(f"  {j}) Time: {s['slot']} | Type: {s['type']} | Mode: {s['mode']}")
            lines.append(f"     Topic: {s['topic']}")
            lines.append(f"     Template: {Path(s['template']).stem}")
            lines.append(f"     Angle: {s['angle']} | Audience: {s['audience']} | Outcome: {s['outcome']}")
        lines.append("")
