...
def load_benchmark_handles_for_types(base: Path, type_codes: list[str]) -> list[str]:
    ...
    seen = set()
    for t in type_codes:
        fname = TYPE_TOPIC_FILES.get(t)
        if not fname:
            continue
        p = base / "strategy" / fname
        if not p.exists():
            continue
...
def build_prompt_bundle(account_root: Path, topic: dict, template_text: str) -> dict:
    account_profile = (account_root / "profile" / "ACCOUNT_PROFILE.md")
    template_rules = (account_root / "strategy" / "TEMPLATE_RULES.md")
    profile_text = account_profile.read_text(encoding="utf-8") if account_profile.exists() else ""
    rules_text = template_rules.read_text(encoding="utf-8") if template_rules.exists() else ""
...
def main() -> int:
    ...
    pause_flag = cfg.base / 'automation' / 'PAUSE_GENERATION'
    if pause_flag.exists():
        print(f"paused: {cfg.base.name} ({pause_flag})")
        return 0
    date = args.date
