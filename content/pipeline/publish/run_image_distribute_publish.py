...
from content.pipeline.publish.assemble_account_queues import choose_candidates, dump_json, load_json
from content.pipeline.write.article_formatter import validate_article_publish_contract
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--publisher-python", default=str(Path(__file__).resolve().parents[3] / "tools/x-schedule-post/.venv/bin/python"))
    parser.add_argument("--publisher-cli", default=str(Path(__file__).resolve().parents[3] / "tools/x-schedule-post/x_schedule_post/cli.py"))
